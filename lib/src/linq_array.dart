...
extension IterableExtensions<T> on Iterable<T> {
  ...
  IArray<T> toIArray() => length.select(elementAt);
...
extension IArrayExtensions<T> on IArray<T> {
  ...
  IArray<T> repeatElements(int count) => count.select((i) => this[i ~/ count]);
  ...
  IArray<U> select<U>(U Function(T) f) => count.select((i) => f(this[i]));

  /// Returns an array generated by applying a function to each element.
  IArray<U> selectByIndex<U>(U Function(T, int) f) =>
      count.select((i) => f(this[i], i));

  /// Returns an array generated by applying a function to each element.
  IArray<U> selectIndices<U>(U Function(int) f) => count.select(f);
  ...
  IArray<V> zip<U, V>(IArray<U> other, V Function(T, U) f) =>
      math.min(count, other.count).select((i) => f(this[i], other[i]));

  /// Returns an array generated by applying a function to corresponding pairs of elements in both arrays.
  IArray<V> zipByIndex<U, V>(IArray<U> other, V Function(T, U, int) f) =>
      math.min(count, other.count).select((i) => f(this[i], other[i], i));
  ...
  IArray<W> zip2<U, V, W>(
      ...
      math
          ...
          .select((i) => f(this[i], other[i], other2[i]));
  ...
  IArray<W> zip2ByIndex<U, V, W>(
      ...
      math
          .min(math.min(count, other.count), other2.count)
          .select((i) => f(this[i], other[i], other2[i], i));
  ...
  IArray<X> zip3<U, V, W, X>(IArray<U> other, IArray<V> other2,
      ...
      math
          ...
          .select((i) => f(this[i], other[i], other2[i], other3[i]));
  ...
  IArray<X> zip3ByIndex<U, V, W, X>(IArray<U> other, IArray<V> other2,
      ...
      math
          .min(math.min(count, other.count),
              math.min(other2.count, other3.count))
          .select((i) => f(this[i], other[i], other2[i], other3[i], i));
  ...
  IArray<T> stride(int n) => (count ~/ n).select((i) => this[i * n % count]);
  ...
  IArray<T> prepend(T x) => (count + 1).select((i) => i == 0 ? x : this[i - 1]);
  ...
  IArray<IArray<T>> splitByIndices(IArray<int> indices) => indices
      .prepend(0)
      .zip(indices.append(count), (int x, int y) => slice(x, y));
  ...
  /// Forces evaluation (aka reification) of the array the separate isolate.
  Future<IArray<T>> evaluateInIsolate() => compute(_evaluateIsolate, this);
  static IArray<T> _evaluateIsolate<T>(IArray<T> arr) => arr.evaluate();

  /// Converts to a regular array in the separate isolate.
  Future<List<T>> toListInIsolate() =>
      count == 0 ? Future.value(<T>[]) : compute(_toListIsolate, this);
  static List<T> _toListIsolate<T>(IArray<T> arr) => arr.toList();

  /// Maps pairs of elements to a new array.
  IArray<U> selectPairs<U>(U Function(T, T) f) =>
      (count ~/ 2).select((i) => f(this[i * 2], this[i * 2 + 1]));

  /// Maps every 3 elements to a new array.
  IArray<U> selectTriplets<U>(U Function(T, T, T) f) => (count ~/ 3)
      .select((i) => f(this[i * 3], this[i * 3 + 1], this[i * 3 + 2]));

  /// Maps every 4 elements to a new array.
  IArray<U> selectQuartets<U>(U Function(T, T, T, T) f) => (count ~/ 4).select(
      (i) => f(this[i * 4], this[i * 4 + 1], this[i * 4 + 2], this[i * 4 + 3]));
